...
export const featureProductsSchema = IProduct.concat(Yup.object({
...
export const featureNewProductSchema = IProduct.concat(Yup.object({
  position: Yup.number().min(1, "Minimum number is one").max(15, "Maximum number is 15").required("Position is Required"),
}));

export type IHomeFeatureNewProduct = Yup.InferType<typeof featureNewProductSchema>;
